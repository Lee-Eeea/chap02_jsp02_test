...
<body>
	<pre>
		...
		1. 제어를 다른 페이지로 이동시킨다. 즉, 다른 페이지로 이동.
		2. 이 페이지의 내용은 웹브라우저에 나오지 않는다.
		   왜냐하면 forward 페이지로 이동하기 전에 현재 페이지에서
		   생성되어서 버퍼에 저장되었던 내용을 버리기 때문이다.
		3. jsp:forward 액션 태그 뒤에 오는 코드는 실행되지 않는다.
		4. jsp:forward 액션 태그의 단점은 현재 페이지의 URL주소가 아니라
		   이동해간 페이지의 주소가 표시된다는 것이다. (source.jsp가 아니라 target.jsp가 타이틀)
